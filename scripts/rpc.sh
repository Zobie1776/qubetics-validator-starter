...
rpc_status() {
  ...
  if (( HAVE_UFW )); then
    ufw_26656=$(ufw status numbered 2>/dev/null | grep -E '26656' || true)
    ufw_26657=$(ufw status numbered 2>/dev/null | grep -E '26657' || true)
  else
...
rpc_expose() {
  ...
  apt-get install -y nginx ssl-cert >/dev/null
...
generate_domain_site() {
  ...
  {
    echo "limit_req_zone $binary_remote_addr zone=rpc:10m rate=5r/s;"
    echo "server {"
    ...
    echo "        deny all;"
    echo "        limit_req zone=rpc burst=10 nodelay;"
...
case "$COMMAND" in
  ...
  lockdown)
    ...
    ;;
  *)
    echo "Usage: $0 {status|local|expose|domain|lockdown}"
    exit 1
    ;;
esac
